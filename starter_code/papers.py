    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        if all_papers:
            with open(DATA_FILE, newline='') as f:
                papers = list(csv.reader(f))[1:]
                papers_dict = _build_nested_dict(papers, by_year)
                subtrees = _build_tree_from_dict(papers_dict)
...
def _build_nested_dict(data: List[List[str]], by_year: bool) -> Dict:
    """
        Construct a nested dictionary from a list of paper data.

        Each paper is represented by a list of strings, where each string
        corresponds to a specific attribute of the paper (e.g., title, author).
        This function organizes these papers into a nested dictionary structure,
        which can then be
        used to construct a tree of PaperTree objects.

        The papers are categorized into hierarchical levels, starting with year
        (if by_year is True), then by each subsequent category found in the
        CATEGORY_IDX of the paper data.
        """
    nested_dict = {}
    for line in data:
        lst = []
        if by_year:
            lst.append(line[YEAR_IDX].strip())
        categories = line[CATEGORY_IDX].split(':')
        for category in categories:
            lst.append(category.strip())
        current = nested_dict
        for category in lst:
...
def _build_tree_from_dict(nested_dict: Dict, path: str = '') -> List[
        'PaperTree']:
    """
    Recursively build a list of PaperTree objects from a nested dictionary.

    The nested dictionary is assumed to be structured with keys representing
    categories and 'papers' lists containing paper data dictionaries at the
    leaves.
    Each recursive call constructs a PaperTree node for a category or a paper,
    and calls itself to construct subtrees for nested categories.
    """
    trees = []
    for key in nested_dict:
        if key == 'papers':
            ...
            subtrees = _build_tree_from_dict(nested_dict[key], path + key + '/')
